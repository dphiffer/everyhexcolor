...
class Twitter {
  ...
  function setup_request($method, $path, $args) {
    ...
    if (strpos($path, 'statuses/update_with_media') !== false) {
      $update_with_media = true;
      $args = $this->upload_media($args);
      $this->authorize_request($method, $path, array());
    } else {
      $this->authorize_request($method, $path, $args);
    }
    if ($method === 'GET') {
